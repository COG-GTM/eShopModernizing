...
- Client libraries (Bootstrap 5.1, jQuery 3.6, jQuery Validation + Unobtrusive) are loaded from jsDelivr CDN rather than vendored under `wwwroot/lib`, keeping the repo free of ~75k lines of third-party minified assets.

## How this was delivered

The port is split into three stacked PRs so each is reviewable on its own and builds independently:

1. Project scaffold — `.csproj`, `Program.cs`, layout, config, error page.
2. Data + services — models, EF Core `CatalogDbContext`, seed data, `ICatalogService` real/mock implementations, DI wiring.
3. Catalog pages — Razor Pages CRUD + list/pagination, product images, these notes.
